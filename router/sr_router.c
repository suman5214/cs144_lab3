...
void sr_arp_reply_send_pending_packets(struct sr_instance *sr,
                                       struct sr_arp_requet *arp_requet,
                                       uint8_t *dhost,
                                       uint8_t *shost,
                                       struct sr_if *iface)
{
                               ...
                               uint8_t *ipPacket)
{
  ...
  if (lpmEntry != NULL)
  {
    ...
    else
    {
      ...
      struct sr_arp_requet *arp_requet = sr_arpcache_queuereq(&(sr->cache),
                                                      nextHopIP,
                                                      packet,
                                                      icmpPacketLen,
                                                      &(interface->name));
      handle_arp_requet(sr, arp_requet);
    }
  }
                          ...
                          char *interface)
{
  ...
  else if (op == arp_op_reply)
  {
    ...
    struct sr_arp_requet *arp_requet = sr_arpcache_insert(&(sr->cache), senderHardAddr, senderIP);
    if (!arp_requet)
    {
      printf("****** -> Send outstanding packets.\n");

      struct sr_packet *unsend_packet = arp_requet->packets;

      while (unsend_packet != NULL)
      {
        sr_ethernet_hdr_t *eth_hdr = (sr_ethernet_hdr_t *)unsend_packet->buf;
        memcpy(eth_hdr->ether_shost, myInterface->addr, ETHER_ADDR_LEN);
        memcpy(eth_hdr->ether_dhost, senderHardAddr, ETHER_ADDR_LEN);

        sr_send_packet(sr, unsend_packet->buf, unsend_packet->len, myInterface);
        unsend_packet = unsend_packet->next;
      }
      sr_arp_requet_destroy(&(sr->cache), arp_requet);
    }
  ...
  if (myInterface == NULL)
  {
    ...
    else
    {
      ...
      else
      {
        ...
        struct sr_arp_requet *nextHopIParp_requet = sr_arpcache_queuereq(&(sr->cache), nextHopIP, packet, len, &(lpmEntry->interface));
        handle_arp_requet(sr, nextHopIParp_requet);
      }
    }
  }
  else
